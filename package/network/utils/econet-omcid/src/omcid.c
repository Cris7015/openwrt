...
static int allow_selfcreate;    /* opt-in only: OMCID_ALLOW_SELFCREATE=1 enables the
                                 * OLT-specific 266->268 GEM-CTP experiment */
...
int main(int argc,char**argv){
    ...
    const char *experimental=getenv("OMCID_EXPERIMENTAL");
    t0=now_ms();
    if(!experimental || experimental[0]!='1'){
        LOG("REFUSED: this H660-style OMCI prototype requires OMCID_EXPERIMENTAL=1");
        return 2;
    }
    { const char*s = argc>2 ? argv[2] : getenv("OMCID_SERIAL");
      if(!parse_onu_sn(s)){
          LOG("REFUSED: pass a valid runtime serial as argv[2] or OMCID_SERIAL");
          return 2;
      } }
    { const char*e=getenv("OMCID_ALLOW_SELFCREATE"); if(e && e[0]=='1') allow_selfcreate=1; }
    ...
    LOG("omcid experimental H660 profile up on %s (ifindex %d) selfcreate=%d",ifn,ifr.ifr_ifindex,allow_selfcreate);
    seed_mib();
    ...
    for(;;){
        ...
        if(n<0){
            ...
            LOG("recvfrom FAIL %s",strerror(errno));
            break;
        }
        if(n<OMCI_LEN) continue;

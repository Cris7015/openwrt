...
#include <linux/math64.h>	/* PORT 6.18: div_u64() for the fixed-point TOD calc */

#include "common/drv_global.h"
#include "gpon/gpon_dev.h"
#include "common/phy_if_wrapper.h"
#include "gpon/gpon_power_management.h"	/* PORT: modify_sleep_count() prototype */
...
int gponDevGetPloamMsg(PLOAM_RAW_General_T *pPloamMsg)
{
	...
	if(gponPloamStatus.Bits.ploamd_fifo_used == 0) {
		...
		for(i=0 ; i<3 ; i++) {
			/* BYTE-ORDER: the FIFO word packs ONU-ID/Msg-ID in the HIGH bytes
			 * (bits 31:24 / 23:16), while PLOAM_RAW_General_T.raw reads
			 * dest_id/msg_id from value[] byte0/byte1.
			 *
			 * On little-endian byte0 is bits 7:0, so the word has to be swapped
			 * or every DS PLOAM decodes as an unknown message and the ONU stays
			 * at O2. On BIG-endian (this SoC, MIPS 34Kc BE) byte0 already IS
			 * bits 31:24, so the swap is not just unnecessary -- it breaks a
			 * layout that was already correct. Measured here against a live
			 * Movistar OLT: with the unconditional swab32 the broadcasts decoded
			 * as msg_id 0x20/0x29, which do not exist in G.984.3; without it they
			 * come out as onu_id=ff msg_id=01 (Upstream_Overhead) and msg_id=14
			 * (Extended_Burst_Length), which is exactly what an OLT sends to
			 * unregistered ONUs. */
			{
				u32 w = IO_GREG(G_PLOAMd_RDATA) ;
		...
		/* DIAG (session 7, O2->O3): log every downstream PLOAM as read (post-swap). */
		{ extern int ploam_verbose;
		if(ploam_verbose || pPloamMsg->raw.dest_id != 0xff)   /* 2026-07-08: log DIRECTED msgs ALWAYS (flood-free: skip onu_id=ff broadcasts) to capture the full OLT->us sequence around the Config_PortID deactivate */
		printk(KERN_DEBUG "PLOAM-DS: raw %08x %08x %08x | onu_id=%02x msg_id=%02x | act_st=O%u\n",
			pPloamMsg->value[0], pPloamMsg->value[1], pPloamMsg->value[2],
			pPloamMsg->raw.dest_id, pPloamMsg->raw.msg_id,
			IO_GREG(0xbfb640bcu) & 0xf) ; }
	} else {
		return -EFAULT ;
	}
...
int gponDevSendPloamMsg(PLOAM_RAW_General_T *pPloamMsg, uint times)
{
	...
	{
	extern int tx_ploam_swab ;
	uint w[3] ;
	for(j=0 ; j<3 ; j++)
		w[j] = tx_ploam_swab ? swab32(pPloamMsg->value[j]) : pPloamMsg->value[j] ;
	/* 2026-07-08 A/B: STOCK writes value[] DIRECTLY (no swab); the port's symmetric TX swab is UNVALIDATED and
	 * first hits the wire at the O5 Password/ACK (= where OLT deactivates). Log the exact on-wire words. */
	printk(KERN_DEBUG "PLOAMu-TX swab=%d val=[%08x %08x %08x] wire=[%08x %08x %08x] x%u\n",
		tx_ploam_swab, pPloamMsg->value[0], pPloamMsg->value[1], pPloamMsg->value[2], w[0], w[1], w[2], times) ;
	for(i=0 ; i<times ; i++) {
		for(j=0 ; j<3 ; j++) {
			IO_SREG(G_PLOAMu_WDATA, w[j]) ;
		}
	}
	}
...
int gponDevSetNewTod(uint superframe, uint sec, uint nanosec) 
{
	...
	uint delta = 0;
	u64 tod_tmp = 0;
	/* PORT 6.18: kernel has no FP runtime.  Original float math:
	 *   n1310=1.4677, n1490=1.4682, up=1.24416, onuResponseTime=35000
	 *   delta = ((eqd/up) + onuResponseTime) * n1490/(n1310+n1490)
	 * Re-expressed in integer fixed-point (up x1e5; n14xx x1e4):
	 *   eqd/up                 -> eqd*100000/124416
	 *   *n1490/(n1310+n1490)   -> *14682/29359  (~0.50008) */

	PON_MSG(MSG_DBG,"Before gponDevSetNewTod sec = %x, nanosec = %x\n", sec, nanosec);
	tod_tmp  = div_u64((u64)gpGponPriv->gponCfg.eqd * 100000ULL, 124416ULL);
	tod_tmp += 35000ULL;
	tod_tmp  = div_u64(tod_tmp * 14682ULL, 29359ULL);
	delta    = (uint)tod_tmp;
...
int gponDevEnableSniffMode(XPON_Mode_t enable)
{
	...
	return 0;	/* PORT 6.18: -Werror=return-type */
}
...
{	
	...
	if(mode == XPON_ENABLE) {
		...
		regWrite32(0xBFB51528u, 0);   /* ★ real clear of GDM2 enabled_rx_channels (FE_API above is a no-op) */
		regWrite32(0xBFB51524u, 0);   /* ★ real clear of GDM2 enabled_tx_channels */
	} else {
		...
		/* ★ M1 Step-4 (2026-07-08): the FE_API_* above is a no-op __ECNT_HOOK stub on k6.18, so GPON WAN
		 * RX (incl. the OMCC OMCI) never leaves GDM2 for the qdma[1] CPU ring -> OMCI rx=0 -> O5 deactivate.
		 * Program GDM2 directly (regWrite32/KSEG1). Offsets from econet-eth en751221_regs: GDM2 base 0xBFB51400. */
		{
			unsigned int fwd = regRead32(0xBFB51500u);
			/* fwd_cfg: default/mcast/bcast/mymac FPORT[3:0/7:4/11:8/15:12] = 5 (ETX_FPORT_QDMA1_CPU) + drop_oversize(bit25) */
			regWrite32(0xBFB51500u, (fwd & ~0x0000FFFFu) | 0x00005555u | (1u << 25));
			regWrite32(0xBFB51514u, 0x07D0003Cu);                          /* rx_len_threshold: oversize 2000 / runt 60 */
			regWrite32(0xBFB51528u, regRead32(0xBFB51528u) | 0x00000003u); /* enabled_rx_channels: ch0,ch1 */
			regWrite32(0xBFB5140Cu, regRead32(0xBFB5140Cu) | 0x00000003u); /* ★ CDMA2 HWF chan-enable ch0,ch1 = fe_set_hwfwd_channel(CDMA2) per fe_core.ko disasm @0x41d8 */
			printk(KERN_DEBUG "OMCIDBG: GDM2 RX->CPU wired: fwd %08x->%08x rxch=%08x chen=%08x omci_id=%08x\n",
				fwd, regRead32(0xBFB51500u), regRead32(0xBFB51528u),
				regRead32(0xBFB5140Cu), regRead32(0xBFB64048u));
		}
	}
#else
	if(mode == XPON_ENABLE) {
...
int gponDevCheckTContReg(ushort channelId, ushort allocId)
{
	...
	/* PORT 6.18: INREG32(x)==(x), so G_TCONT_ID_0_1 is a (REG_G_TCONT_ID_0_1 *);
	 * keep TcontReg a pointer so the per-register stride arithmetic stays valid
	 * (-Werror=int-conversion).  Each register packs 2 T-CONTs -> channelId/2. */
	volatile REG_G_TCONT_ID_0_1 *TcontReg = NULL;

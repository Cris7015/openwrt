...
int phy_trans_tx_setting(PPHY_TransConfig_T trans_status)
{
	...
	write_data = read_data;
	{
		extern int tx_laser_off;   /* laser hold: never let the config drop bit7 */
		if (tx_laser_off)
			write_data |= (0x01 << 7);
	}
...
int phy_tx_burst_counter(void)
{
	...
	return read_data;		/* TODO port: was missing return (vendor relied on -w) */
#else
	return 0;
#endif /* CONFIG_USE_MT7520_ASIC */
}
...
{
	extern int tx_laser_off;   /* phy_init.c: 1 = laser hold, bit7 must stay set */
	uint read_data = 0;
	...
	{
		/* bit7 clear = burst-enable un-inverted = laser CAN key. The xmcs/GPON
		 * activation path calls this after lock; with the laser held it was the
		 * runtime writer flipping the hold off every activation (seen live as
		 * xpon_setting 0x18f -> 0x10f). Refuse while tx_laser_off. */
		if (tx_laser_off) {
			printk(KERN_INFO "phy_trans_burst_invert: DISABLE skipped (tx_laser_off=1, hold kept)\n");
			return;
		}
		read_data = IO_GPHYREG(PHY_CSR_XPON_SETTING);
		read_data &= ~(0x01 << 7);

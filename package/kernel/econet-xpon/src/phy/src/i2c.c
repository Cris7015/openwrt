 ...
 * The vendor code used create_proc_entry()+pde->write_proc, but on 6.18
 * struct proc_dir_entry is opaque and proc_create() takes struct proc_ops.
 * Adapt the legacy single-shot read_proc / write_proc callbacks. */
typedef int (*i2c_old_read_proc_t)(char *page, char **start, off_t off,
				   int count, int *eof, void *data);
typedef int (*i2c_old_write_proc_t)(struct file *file, const char *buffer,
				    unsigned long count, void *data);
struct i2c_proc_compat { i2c_old_read_proc_t rp; i2c_old_write_proc_t wp; void *data; };

static ssize_t i2c_proc_compat_read(struct file *f, char __user *ubuf,
				    size_t cnt, loff_t *pos)
{
	struct i2c_proc_compat *pc = pde_data(file_inode(f));
...
static ssize_t i2c_proc_compat_write(struct file *f, const char __user *ubuf,
				     size_t cnt, loff_t *pos)
{
	struct i2c_proc_compat *pc = pde_data(file_inode(f));
	if (!pc || !pc->wp)
...
static const struct proc_ops i2c_proc_compat_ops = {
	.proc_read  = i2c_proc_compat_read,
	.proc_write = i2c_proc_compat_write,
};
#define I2C_PROC_CREATE(name, parent, rdfn, wrfn) ({			\
	static struct i2c_proc_compat _pc = { .rp = (rdfn), .wp = (wrfn) }; \
	proc_create_data((name), 0, (parent), &i2c_proc_compat_ops, &_pc); })
...
int i2c_init(void)
{
	...
	i2c_proc = I2C_PROC_CREATE("i2c", NULL, NULL, i2c_write_proc) ;
	(void)i2c_proc;
    return ret;
}

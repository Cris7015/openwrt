 ...
 * the live PON. This unit can't emit a registrable burst anyway (no laser cal). Set 0 (param)
 * only to deliberately transmit. */
int tx_laser_off = 1;      module_param(tx_laser_off, int, 0644);  /* default 1 = laser held OFF (safe). Set 0 to transmit. */

/* ★ LASER-HOLD WATCHDOG (2026-08-07). The hold is only re-applied in gpon_do_lock()'s
 * tail, but every do_lock call site is gated on !gpon_is_locked() -- and the CDR stays
 * locked across rmmod/insmod (warm PHY). So an insmod onto an already-locked CDR never
 * runs do_lock and the init-path 0x10F write (mt7570_init / do_lock preamble) is the
 * LAST thing to touch bit7 => hold down, stable (seen live: xpon_setting 0x10f, 4 reads).
 * This helper asserts the hold NOW if tx_laser_off says so; called at insmod, from the
 * keeper every 2-3 s, and after any path known to drop bit7. Counter is readable at
 * /sys/module/econet_xpon/parameters/laser_hold_reasserts -- nonzero means something
 * dropped the hold at runtime (find it and fix it). */
int laser_hold_reasserts = 0; module_param(laser_hold_reasserts, int, 0444);
void xpon_laser_hold_check(const char *who)
{
	uint v;
	if (!tx_laser_off)
		return;
	...
	if (!(v & 0x80u)) {
		...
		laser_hold_reasserts++;
		printk(KERN_WARNING "econet_xpon: LASER HOLD was DOWN (XPON_SETTING=%08x) - re-asserted by %s (count=%d, now=%08x)\n",
		       v, who, laser_hold_reasserts, IO_GREG(0xbfaf0138u));
	}
}
/* GTC hardware sniffer (command '5'). 1 = enable DBG_GTC_ETH_EXTR bit0, which makes
 * the GPON MAC extract frames off the GTC and wrap them in Ethernet (EtherType
 * 0x88b6, per DS/US_GTC_EXTR_ETH_HDR) so they can be read with tcpdump. Receive
 * side only -- it captures the downstream that already arrives, so it needs no TX
 * and is safe to run with the laser held off. */
int sniff_gtc = 0;         module_param(sniff_gtc, int, 0644);
/* Top 16 bits of the DA/SA the MAC stamps on each extracted frame (only those
 * bits are configurable). Broadcast DA by default so the frame is accepted. */
int sniff_da_h16 = 0xffff; module_param(sniff_da_h16, int, 0644);
int sniff_sa_h16 = 0x0200; module_param(sniff_sa_h16, int, 0644);
/* dbg_ds_all_gem_filter: capture every downstream GEM port, not only ours. */
int sniff_all_gem = 1;     module_param(sniff_all_gem, int, 0644);
/* Special tag written into DS/US_GTC_EXTR_ETH_HDR[15:0] ("sp"). On the EPON side
 * the vendor uses a 6-bit LAN port mask here (SNIFFER_SP_TAG_MASK 0x3F), so this
 * most likely selects where the mirrored frame is sent. -1 = leave as-is. */
int sniff_sp = -1;         module_param(sniff_sp, int, 0644);
/* Command '7': open/close GDM2's RX/TX channels (0xBFB51528 / 0xBFB51524).
 * gpon_init() calls gponDevMbiStop(), which zeroes both, and only the O4->O5
 * transition reopens them -- so anything the GTC extractor produces in O3 has
 * nowhere to go. 1 = set channels 0,1; 0 = back to zero (revert). */
int gdm2_rxch = 0;         module_param(gdm2_rxch, int, 0644);
/* Channel mask used by command '7'. gdm_rx_ch_en has one bit per RX channel
 * (0..15); the original probe only opened 0x3, so a mirror arriving on any
 * other channel is counted by GDM2 and then refused. Set 0xffff to open all. */
uint gdm2_chmask = 0x3u;   module_param(gdm2_chmask, uint, 0644);
...
int pon_phy_init(void)
{
	...
	/* ★ ROOT-CAUSE FIX for the unstable laser hold: assert it at insmod, unconditionally.
	 * If the CDR is already locked (warm PHY across rmmod/insmod) no do_lock ever runs,
	 * so this is the ONLY place that applies the hold on that path. */
	xpon_laser_hold_check("insmod");
...
/* Minimum EN7571 revision to accept. The original check required rev >= 3,
 * which rejects the XR500v: its front-end reads id=3 rev=1 (same part, older
 * silicon). Failing the check makes en7571_optical_bringup() bail out before
 * biasing the APD, so the LOS comparator still sees light -- fibre in/out does
 * flip reg 0x133 bit0 -- but the receiver never reaches its operating point and
 * rx_sync stays 0. Lowered to 1 so this board is accepted; raise it back if an
 * older revision turns out to need a different init sequence. */
static int en7571_min_rev = 1;
module_param(en7571_min_rev, int, 0644);
...
static int en7571_detect(void)
{
	...
	if (e7_rd(0x170, &id, 1) || e7_rd(0x15C, &rev, 1))
		return 0;
	printk(KERN_INFO "en7571: BOSA id=%u rev=%u (need id=3 rev>=%d)\n",
	       id, rev, en7571_min_rev);
	return (id == 3) && (rev >= en7571_min_rev);
}
...
int en7571_tx_emitting(void)
{
	...
	xpon_laser_hold_check("tx_emitting");   /* the measurement leaves bit7=0; restore the hold */
	return (lit - dark > 8);
}
...
int en7571_tx_ramp_bias(void)
{
	...
	xpon_laser_hold_check("tx_ramp_bias");   /* ramp ran burst-capable; restore the hold */
	return 0;
}
...
int gpon_do_lock(int quiet)
{
	...
	/* keep bit7 (laser hold) asserted THROUGH the sequence when tx_laser_off: the old
	 * bare 0x10F opened a laser-capable window until the tail re-set it, and any path
	 * that skipped the tail left it down for good. Lock works with bit7=1 (TX-only bit). */
...
static int gpon_lock_keeper(void *arg)
{
	...
	while (!kthread_should_stop()) {
		xpon_laser_hold_check("keeper");   /* laser-hold watchdog: every 2-3 s */
...
static ssize_t xpon_los_read(struct file *f, char __user *b, size_t l, loff_t *o)
{
	...
	if (tx_laser_off)
		setting |= 0x80u;   /* laser hold: never restore a stale bit7=0 snapshot */
...
static ssize_t xpon_los_write(struct file *f, const char __user *b, size_t l, loff_t *o)
{
	...
	if (c == '7') {		/* open/close GDM2 RX+TX channels -- the gate gponDevMbiStop() shuts.
				 * echo <0|1> > /sys/module/econet_xpon/parameters/gdm2_rxch ; echo 7 > /proc/econet_xpon_los
				 * Reversible: writing 0 puts them back exactly as gpon_init() left them.
				 * WARNING: opening the PON datapath has been seen to take the LAN MAC
				 * down on this SoC -- keep a serial console attached. */
		uint rx0 = regRead32(0xBFB51528u), tx0 = regRead32(0xBFB51524u);
		if (gdm2_rxch) {
			regWrite32(0xBFB51528u, rx0 | gdm2_chmask);
			regWrite32(0xBFB51524u, tx0 | gdm2_chmask);
		} else {
			regWrite32(0xBFB51528u, rx0 & ~gdm2_chmask);
			regWrite32(0xBFB51524u, tx0 & ~gdm2_chmask);
		}
		printk(KERN_DEBUG "7: GDM2 rxch %08x->%08x txch %08x->%08x (want=%d) | RXcnt48=%08x\n",
			rx0, regRead32(0xBFB51528u), tx0, regRead32(0xBFB51524u),
			!!gdm2_rxch, regRead32(0xBFB51648u));
		return l;
	}
	if (c == '8') {		/* ★ GDM2 drop-filter gate. The GTC extractor fabricates Ethernet frames --
				 * synthetic DA/SA (only the high 16 bits are configurable) plus
				 * ethertype 0x88b6 wrapped around a GPON payload -- so they carry no
				 * valid FCS and can exceed oversize_len. fwd_cfg bits 25..20
				 * (DROP_OVERSIZE/RUNT/CRC/IP4_CSUM/TCP_CSUM/UCP_CSUM) discard them
				 * AFTER the RX counters have already ticked, which is why GDM2 counts
				 * ~8k/s while the QDMA1 rings never see a descriptor.
				 * Clears those six bits; run again to restore. GDM2 only: GDM1 (LAN)
				 * is a separate register, so this cannot take the LAN down. */
		static uint fwd_saved;
		static int  fwd_cleared;
		uint fwd = regRead32(0xBFB51500u);

		if (!fwd_cleared) {
			fwd_saved = fwd;
			regWrite32(0xBFB51500u, fwd & ~0x03F00000u);
			fwd_cleared = 1;
		} else {
			regWrite32(0xBFB51500u, fwd_saved);
			fwd_cleared = 0;
		}
		printk(KERN_DEBUG "8: GDM2 fwd %08x -> %08x (drop filters %s) rxlen=%08x oversize=%u runt=%u\n",
			fwd, regRead32(0xBFB51500u), fwd_cleared ? "OFF" : "RESTORED",
			regRead32(0xBFB51514u),
			(regRead32(0xBFB51514u) >> 16) & 0xffffu,
			regRead32(0xBFB51514u) & 0xffffu);
		return l;
	}
	if (c == '6') {		/* M1 Step-4 diag: dump GDM2 FE datapath regs + RX counters + OMCC latch.
				 * WAS 'G', which is unreachable: an earlier 'G' (MAC-side GPON
				 * activation, ~line 2829) returns first. Moved to '6' so the dump
				 * can actually be run. Read-only. */
	...
	if (c == '5') {		/* GTC hardware sniffer on/off. The GPON MAC can extract frames off the
				 * GTC and wrap them in Ethernet (EtherType 0x88b6) -- a mirror port in
				 * silicon. Set sniff_gtc then: echo 5 > /proc/econet_xpon_los
				 * Regs are at GPON base 0x1FB60000 (KSEG1 0xBFB6xxxx):
				 *   0x4368 DBG_GTC_ETH_EXTR  bit0=enable bit8=pkt_padding bit31=dbg_tx_rst
				 *   0x4374 DS_GTC_EXTR_ETH_CNT   0x4378 US_GTC_EXTR_ETH_CNT
				 * RX-side only: captures the downstream that already arrives, so this
				 * works with the laser held off. */
		uint was = IO_GREG(0xbfb64368u), now, gem, damac;

		if (sniff_gtc) {
			/* The MAC wraps each extracted frame in Ethernet, but only the TOP
			 * 16 bits of the DA/SA are configurable (the rest is filled by the
			 * hardware, presumably with capture metadata). Leaving them at 0
			 * builds a header nothing will accept, which is why the counters
			 * moved but no netdev ever saw a packet. Broadcast DA so whatever
			 * the frame lands on will take it. */
			IO_SREG(0xbfb64390u, ((uint)sniff_da_h16 << 16) | (uint)sniff_sa_h16);
			IO_SREG(0xbfb6438cu, ((uint)sniff_da_h16 << 16) | (uint)sniff_sa_h16);
			/* dbg_ds_all_gem_filter (bit31): capture EVERY downstream GEM port,
			 * not just ours. bit27 would exclude OMCI -- leave it off. */
			gem = IO_GREG(0xbfb6437cu);
			IO_SREG(0xbfb6437cu, sniff_all_gem ? (gem | 0x80000000u)
							  : (gem & ~0x80000000u));
			IO_SREG(0xbfb64398u, 0x1u);		/* sniff_cnt_en */
			if (sniff_sp >= 0) {			/* set the mirror destination tag */
				uint h = IO_GREG(0xbfb6436cu);
				IO_SREG(0xbfb6436cu, (h & 0xffff0000u) | ((uint)sniff_sp & 0xffffu));
				h = IO_GREG(0xbfb64370u);
				IO_SREG(0xbfb64370u, (h & 0xffff0000u) | ((uint)sniff_sp & 0xffffu));
			}
			IO_SREG(0xbfb64368u, was | 0x1u);	/* dbg_gtc_eth_extr_en */
		} else {
			IO_SREG(0xbfb64368u, was & ~0x1u);
		}

		now   = IO_GREG(0xbfb64368u);
		gem   = IO_GREG(0xbfb6437cu);
		damac = IO_GREG(0xbfb64390u);
		printk(KERN_DEBUG "5: EXTR %08x->%08x en=%d | GEMFLT %08x (all_gem=%d) | RX_DA_SA %08x | CNT_EN %08x\n",
			was, now, !!sniff_gtc, gem, !!(gem & 0x80000000u), damac,
			IO_GREG(0xbfb64398u));
		printk(KERN_DEBUG "5: DBG_REG=%08x US_FIFO=%08x HDRWIDO=%08x\n",
			IO_GREG(0xbfb64380u), IO_GREG(0xbfb64384u), IO_GREG(0xbfb64388u));
		printk(KERN_DEBUG "5: DS_CNT=%u US_CNT=%u | hdr ds=%08x us=%08x | TX_RX_PID=%08x RX_TX_TPID=%08x\n",
			IO_GREG(0xbfb64374u), IO_GREG(0xbfb64378u),
			IO_GREG(0xbfb6436cu), IO_GREG(0xbfb64370u),
			IO_GREG(0xbfb64394u), IO_GREG(0xbfb6439cu));
		return l;
	}
	...
	if (c == 'B') {		/* ★ TX laser emission check + bias ramp. en7571_tx_emitting() = dark vs lit
		...
		int emit;
		if (tx_laser_off) {   /* refuse: tx_emitting clears bit7 to measure and the ramp RAISES Ibias = emits */
			printk(KERN_WARNING "B: REFUSED - tx_laser_off=1 (laser held). Clear tx_laser_off first if you REALLY mean to emit.\n");
			return l;
		}
		emit = en7571_tx_emitting();
		if (!emit) {

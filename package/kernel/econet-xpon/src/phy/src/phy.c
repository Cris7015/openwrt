...
/* ---- 2.6.36 -> 6.18 proc_fs compat (compile-milestone) ----
 * struct proc_dir_entry is opaque now; adapt the legacy single-shot
 * read_proc / write_proc callbacks to struct proc_ops. */
typedef int (*phy_old_read_proc_t)(char *page, char **start, off_t off,
				   int count, int *eof, void *data);
typedef int (*phy_old_write_proc_t)(struct file *file, const char *buffer,
				    unsigned long count, void *data);
struct phy_proc_compat { phy_old_read_proc_t rp; phy_old_write_proc_t wp; void *data; };

static ssize_t phy_proc_compat_read(struct file *f, char __user *ubuf,
...
static ssize_t phy_proc_compat_write(struct file *f, const char __user *ubuf,
				     size_t cnt, loff_t *pos)
{
	struct phy_proc_compat *pc = pde_data(file_inode(f));
	if (!pc || !pc->wp)
		return -EIO;
	return pc->wp(f, (const char __force *)ubuf, cnt, pc->data);
}
static const struct proc_ops phy_proc_compat_ops = {
	.proc_read  = phy_proc_compat_read,
	.proc_write = phy_proc_compat_write,
};
#define PHY_PROC_CREATE(name, parent, rdfn, wrfn) ({			\
	static struct phy_proc_compat _pc = { .rp = (rdfn), .wp = (wrfn) }; \
	proc_create_data((name), 0, (parent), &phy_proc_compat_ops, &_pc); })

/* TODO port: PHY_START_TIMER macro used struct timer_list.data (removed) and
 * timer callbacks now take (struct timer_list *).  phy_event_poll() keeps its
 * legacy prototype (phy.h); provide a modern-signature wrapper for timer_setup. */
void phy_event_poll_timer(struct timer_list *t)
{
	(void)t;
	phy_event_poll(0);
}

/* TODO port: cross-file fns defined in mt7570.c with no header prototype */
extern void set_flash_register_EnvTemp(uint temperature_input);
extern void mt7570_show_LUT(void);
extern void DOL_flash_dump(void);
extern void LOS_calibration(int LOS_thld_H, int LOS_thld_L);
extern void Ibias_temperature_set(void);

/* TODO port: userspace rand() is unavailable in-kernel; debug PRBS test only */
static int rand(void) { return (int)get_random_u32(); }
...
{
	uint buffer;
	/* PORT: gpPhyPriv is allocated asynchronously by the xpon daemon; a bring-up
	 * call (phy_dev_init) issued before that races to a NULL deref at
	 * gpPhyPriv->i2c_u2_clk_div (offset 0x54). Fall back to the SIF clk-div constant. */
	ushort clkdiv = gpPhyPriv ? gpPhyPriv->i2c_u2_clk_div : I2C_U2_CLK_DIV;

	phy_I2C_read(0, clkdiv, U1_DEV_ADDR, 2, sub_addr, (unchar *)&buffer, 4);
...
void phy_I2C_write_translet(uint sub_addr, uint val)
{
	...
	ushort clkdiv = gpPhyPriv ? gpPhyPriv->i2c_u2_clk_div : I2C_U2_CLK_DIV;   /* NULL-race guard */
	...
	phy_I2C_write(0, clkdiv, U1_DEV_ADDR, 2, sub_addr, (unchar *)&buffer, 4);
}
...
void phy_trans_model_setting(void)
{
	...
	else if(strcmp(vendor_id, "SOURCEPHOTONICS ") == 0)
	{
		if(strcmp(vendor_pn, "SFA3424THPCDFJ  ") == 0)
			{ extern int tx_laser_off; IO_SPHYREG(PHY_CSR_XPON_SETTING, 0x14f | (tx_laser_off ? 0x80 : 0)); }
...
#endif
{
	/* PORT: the daemon's get-phy-params job can poll transceiver status before the
	 * phy private data is allocated -> gpPhyPriv NULL -> deref at 0x54 (i2c_u2_clk_div)
	 * -> Oops. Guard it (skip the read until the phy is initialized). */
	if (!gpPhyPriv || !transceiver_param)
		return;
	#if defined(CONFIG_USE_MT7520_ASIC) || defined(CONFIG_USE_A60928)
		//unchar ptr[2];
		//ushort u2ByteCnt;
#if 1
		phy_I2C_read(0, gpPhyPriv->i2c_u2_clk_div, PHY_TRANS_REG_TABLE2, 1, PHY_TRANS_TEMP_HBYTE, (unchar *)transceiver_param, sizeof(PHY_TransParam_T));//read 10byte once
#else
		//temprature
...
#if (defined(CONFIG_USE_MT7520_ASIC) || defined(CONFIG_USE_A60928)) && defined(TCSUPPORT_WAN_EPON)
	/* TODO port: EPON support dropped from this GPON-only build; phy_epon_frame_err_cnt() is not built */
...
int phy_debug_init(void) 
{
	...
	if(phy_proc_dir){
		/* TODO port: create_proc_entry()+pde->{read,write}_proc replaced by
		 * proc_create_data() + the phy_proc_compat adapter (struct proc_ops). */
#ifdef CONFIG_DEBUG
		phy_proc = PHY_PROC_CREATE("debug", phy_proc_dir, phy_read_proc, phy_write_proc);
#endif /* CONFIG_DEBUG */

		phy_info = PHY_PROC_CREATE("info", phy_proc_dir, phy_info_read_proc, NULL);
		phy_proc_ddmi = PHY_PROC_CREATE("ddmi", phy_proc_dir, phy_proc_ddmi_read_proc, NULL);
		phy_proc_temperature = PHY_PROC_CREATE("Temperature", phy_proc_dir, phy_proc_temperature_read_proc, NULL);
		phy_proc_vcc = PHY_PROC_CREATE("VCC", phy_proc_dir, phy_proc_vcc_read_proc, NULL);
		phy_proc_bias = PHY_PROC_CREATE("BiasCurrent", phy_proc_dir, phy_proc_bias_read_proc, NULL);
		phy_proc_modulation = PHY_PROC_CREATE("ModulationCurrent", phy_proc_dir, phy_proc_modulation_read_proc, NULL);
		phy_proc_rssi = PHY_PROC_CREATE("RSSICurrent", phy_proc_dir, phy_proc_rssi_read_proc, NULL);
		phy_proc_mpd = PHY_PROC_CREATE("MPDCurrent", phy_proc_dir, phy_proc_mpd_read_proc, NULL);
		phy_proc_apd_control = PHY_PROC_CREATE("APDVolControl", phy_proc_dir, NULL, phy_proc_apd_control_write_proc);
		phy_rogue = PHY_PROC_CREATE("rogue", phy_proc_dir, phy_rogue_read_proc, NULL);
	}
...
void phy_event_poll(ulong data)
{
    ...
    mod_timer(&gpPhyPriv->event_poll_timer, jiffies + msecs_to_jiffies(1000));
}
